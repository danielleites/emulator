/**
 * ═══════════════════════════════════════════════════════
 *  sym-mugmon20.js  —  Symbol Orchestrator
 * ═══════════════════════════════════════════════════════
 *  Thin wrapper: registration, lifecycle, data routing.
 *  Bootstraps mm20-core.js + plugins dynamically (no hardcoded paths).
 *  All UI logic lives in mm20-plugins/*.js widgets.
 *
 *  Version : 20.1.R1
 *  ES5 only
...
(function (PV) {
    ...
    symbolVis.prototype.init = function (scope, el, $interval, $timeout) {
        ...
        var _extBase = _getOwnBasePath('sym-mugmon20.js', '/PIVision/Scripts/app/editor/symbols/ext/');
        ...
        var _safeWidget = function (selector, widgetName, options, label) {
            ...
            if (!$.fn[widgetName]) {
                ...
                MM.shield.log('orchestrator', 'init:' + label, new Error(widgetName + ' not loaded'));
            ...
            try {
                ...
                MM.shield.log('orchestrator', 'init:' + label, e);
        ...
        var _initWidgets = function () {
            var $ = window.jQuery;
            if (!$) {
                MM.shield.log('orchestrator', '_initWidgets', new Error('jQuery not available'));
                return;
            }
        ...
        var _forwardData = function (parsed) {
            if (_wSiteGrid && window.jQuery.fn.mm20SiteGrid) {
                try { _wSiteGrid.mm20SiteGrid('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:siteGrid', e); }
            }
            if (_wAlerts && window.jQuery.fn.mm20Alerts) {
                try { _wAlerts.mm20Alerts('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:alerts', e); }
            }
            if (_wHeatmap && window.jQuery.fn.mm20Heatmap) {
                try { _wHeatmap.mm20Heatmap('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:heatmap', e); }
            }
            if (_wReports && window.jQuery.fn.mm20Reports) {
                try { _wReports.mm20Reports('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:reports', e); }
            }
            if (_wTags && window.jQuery.fn.mm20Tags) {
                try { _wTags.mm20Tags('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:tags', e); }
            }
            if (_wTagWarehouse && window.jQuery.fn.mm20TagWarehouse) {
                try { _wTagWarehouse.mm20TagWarehouse('option', 'data', parsed); } catch (e) { MM.shield.log('orchestrator', 'forward:tagWarehouse', e); }
            }
        ...
        scope.exportConfig = function () {
            ...
            a.href = url;
            a.download = 'mm20-config-' + MM.formatDate(new Date(), 'date').replace(/\//g, '-') + '.json';
            a.click();
        ...
        var _initTimeout = $timeout(function () {
            ...
            _log('info', 'Mugbalot Monitor v' + MM.VERSION + ' \u05D0\u05D5\u05EA\u05D7\u05DC');
        }, 200);
        ...
        this.dataUpdate = MM.shield.wrap('orchestrator', 'dataUpdate', function (data) {
        ...
        function _bootstrapAndStart() {
            function afterCore(err) {
                ...
                MM.ensureStyle(MM.resolveUrl('sym-mugmon20.css'));
    ...
    var _symBase = (function () {
        ...
        for (i = scripts.length - 1; i >= 0; i--) {
            src = scripts[i].getAttribute('src') || '';
            if (src.indexOf('sym-mugmon20.js') >= 0) {
                return src.substring(0, src.lastIndexOf('/') + 1);
            }
        }
    ...
    var def = {
        typeName: 'mugmon20',
        displayName: '\u05E0\u05D9\u05D8\u05D5\u05E8 \u05DE\u05D5\u05D2\u05D1\u05DC\u05D5\u05EA v20 \u2014 \u05E9\u05E2\u05D5\u05EA \u05E2\u05D1\u05D5\u05D3\u05D4',
        datasourceBehavior: PV.Extensibility.Enums.DatasourceBehaviors.Multiple,
        iconUrl: _symBase + 'icons/mugmon20.png',
        ...
        getDefaultConfig: function () {
            return {
                ...
                // Footer
                FooterText: '\u05E0\u05D5\u05D2\u05D4 \u2014 \u05DE\u05E0\u05D4\u05DC \u05DE\u05E2\u05E8\u05DB\u05EA \u05D4\u05D7\u05E9\u05DE\u05DC | MM20 v20.0.R1',
                ShowFooter: true,
                // Quota
