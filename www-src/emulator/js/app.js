...
(function (window, $) {
    ...
    function _loadScript(src) {
        ...
        // when available so two symbols that share a plugin (e.g. mug*) only
        // produce one <script> tag in the DOM. The legacy script-injection
        // path below is the unconditional fallback for older runtimes that
        // never loaded js/perf/symbol-loader.js.
        if (typeof window !== 'undefined' && window.SymbolLoader && typeof window.SymbolLoader.loadScript === 'function') {
            return window.SymbolLoader.loadScript(src).then(function () {
                ...
                // Resolve (not reject) — missing plugin should not block symbol loading.
            });
        }
    ...
    function _loadSymbol(symName, onLoaded) {
        ...
        if (type === 'v20') {
            ...
            // WOW symbols — load mu20/mm20 core + WOW renderers
            pluginPromises.push(_loadScript(SYMBOLS_BASE + 'mu20-plugins/mu20-core.js'));
            pluginPromises.push(_loadScript(SYMBOLS_BASE + 'wow-plugins/wow-grid-renderer.js'));
            pluginPromises.push(_loadScript(SYMBOLS_BASE + 'wow-plugins/wow-heatmap-renderer.js'));
            if (symName.indexOf('mug') >= 0) {
    ...
    function _renderSymbol(symName, container) {
        ...
        _loadSymbol(symName, function (def, template) {
            ...
            // ── Ensure WOW symbols have a .wow-root mount point ──
            if (!template && !$wrapper[0].querySelector('.wow-root')) {
                var wowMount = document.createElement('div');
                wowMount.className = 'wow-root';
                wowMount.style.cssText = 'width:100%;height:100%;';
                $wrapper[0].appendChild(wowMount);
            }
    ...
    function _applyStyleToSymbol() {
        ...
        // Sync ALL generic config inputs (data-cfg-key) from DOM
        var allCfgInputs = document.querySelectorAll('[data-cfg-key]');
        for (var ci = 0; ci < allCfgInputs.length; ci++) {
            var inp = allCfgInputs[ci];
            var key = inp.getAttribute('data-cfg-key');
            if (!key) continue;
            if (inp.type === 'checkbox') cfg[key] = inp.checked;
            else if (inp.type === 'number') cfg[key] = parseFloat(inp.value) || 0;
